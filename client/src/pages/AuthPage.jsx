...
export default function AuthPage() {
  ...
  async function handleDirectGoogleLogin(selectedEmail, selectedName) {
    ...
    try {
      ...
      const authUser = await googleLogin({
        ...
        name: selectedName || selectedEmail.split("@")[0].replace(/[._]/g, " "),
        avatarUrl: `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(selectedEmail)}`,
    ...
    <main className="auth-page mx-auto flex min-h-[calc(100vh-66px)] max-w-2xl flex-col items-center justify-center gap-8 px-4 py-10 sm:px-6 lg:px-8">
      ...
      {showGoogleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="w-full max-w-md rounded-2xl border border-black/10 bg-white p-6 shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b border-black/5 pb-4">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-base font-bold text-ink">Sign in with Google</h3>
                  <p className="text-xs text-ink/55">Choose an account to continue to SmartTailor</p>
                </div>
              </div>
              <button 
                type="button" 
                onClick={() => setShowGoogleModal(false)} 
                className="grid h-8 w-8 place-items-center rounded-full text-ink/40 hover:bg-black/5 hover:text-ink transition"
              >
                ✕
              </button>
            </div>
            ...
            <div className="mt-4 space-y-2.5">
              <p className="text-xs font-semibold text-ink/70 uppercase tracking-wider">Select Demo Google Account</p>
              
              <button
                type="button"
                onClick={() => handleDirectGoogleLogin("priya.customer@gmail.com", "Priya Sharma")}
                className="flex w-full items-center gap-3 rounded-xl border border-black/10 bg-black/[0.02] p-3 text-left hover:bg-black/[0.05] hover:border-black/20 transition"
              >
                <div className="grid h-10 w-10 place-items-center rounded-full bg-emerald-100 text-emerald-800 font-bold text-sm">
                  PS
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-ink truncate">Priya Sharma</p>
                  <p className="text-xs text-ink/60 truncate">priya.customer@gmail.com</p>
                </div>
                <span className="text-[10px] font-bold uppercase tracking-wider rounded bg-emerald-50 text-emerald-700 px-2 py-0.5 border border-emerald-200">Customer</span>
              </button>

              <button
                type="button"
                onClick={() => handleDirectGoogleLogin("meena.tailor@gmail.com", "Meena Tailor")}
                className="flex w-full items-center gap-3 rounded-xl border border-black/10 bg-black/[0.02] p-3 text-left hover:bg-black/[0.05] hover:border-black/20 transition"
              >
                <div className="grid h-10 w-10 place-items-center rounded-full bg-amber-100 text-amber-800 font-bold text-sm">
                  MT
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-ink truncate">Meena Tailor</p>
                  <p className="text-xs text-ink/60 truncate">meena.tailor@gmail.com</p>
                </div>
                <span className="text-[10px] font-bold uppercase tracking-wider rounded bg-amber-50 text-amber-700 px-2 py-0.5 border border-amber-200">Tailor</span>
              </button>
            </div>

            <div className="my-4 flex items-center gap-3 text-xs text-ink/40">
              <span className="h-px flex-1 bg-black/10" />
              <span>Or enter another Google email</span>
              <span className="h-px flex-1 bg-black/10" />
            </div>
            ...
            >
              <div>
                <label className="text-xs font-semibold text-ink/75 uppercase tracking-wider">Google Email</label>
                <input
                  type="email"
                  required
                  ...
                  className="mt-1 w-full rounded-lg border border-black/15 bg-white px-3.5 py-2.5 text-sm outline-none focus:border-stitch focus:ring-4 focus:ring-stitch/10"
                />
              </div>
              <div>
                <label className="text-xs font-semibold text-ink/75 uppercase tracking-wider">Your Full Name (Optional)</label>
                <input
                  type="text"
              ...
              <div className="flex gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => setShowGoogleModal(false)}
                  className="flex-1 rounded-lg border border-black/15 px-4 py-2.5 text-sm font-semibold text-ink/70 hover:bg-black/5 transition"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={busy || !customGoogleEmail.trim()}
                  className="flex-1 rounded-lg bg-ink px-4 py-2.5 text-sm font-semibold text-white hover:bg-ink/90 transition shadow-sm disabled:opacity-50"
                >
